...
async def _collect_dashboard_stats() -> dict:
    ...
    try:
        ...
        cursor = await db.conn.execute("SELECT COUNT(*) FROM users WHERE status='premium'")
        row = await cursor.fetchone()
        ...
        # Геймификация
        cursor = await db.conn.execute("SELECT AVG(level), AVG(xp), MAX(level) FROM users")
        row = await cursor.fetchone()
        if row:
        ...
        # Реферралы
        cursor = await db.conn.execute("SELECT COUNT(*) FROM referrals")
        row = await cursor.fetchone()
