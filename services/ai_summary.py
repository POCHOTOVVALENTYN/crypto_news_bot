...
{{
    ...
    async def rewrite_for_telegram(
        ...
        """
        # Цели длины с учётом реального overhead (заголовок+цены+футер)
        if tone == "digest":
            target_len = 200   # краткая выжимка для admin preview (без цен и футера)
        elif has_image:
            target_len = 550   # caption: 1024 - ~300 overhead (заголовок+цены+футер) - 174 запас
        else:
            target_len = 3200  # message лимит — 4096, запас на заголовок + футер

        # Тон для коротких текстов — расширяем, а не сокращаем
        is_enrich = (tone == "enrich")
        is_digest = (tone == "digest")

        # Для обычных тонов: если текст уже укладывается — не трогаем
        # Но для digest и breaking — рерайтим всегда
        if not is_enrich and not is_digest and tone != "breaking" and len(text) <= target_len:
        ...
        tone_instructions = {
            ...
            "digest": (
                "Стиль: РЕДАКТОРСКАЯ ВЫЖИМКА. Ровно 2-3 коротких предложения.\n"
                "— Предложение 1: главный факт (кто, что, цифры)\n"
                "— Предложение 2: контекст или причина\n"
                "— Предложение 3 (опц.): вывод для инвестора\n"
                "Никаких списков, никаких эмодзи, только текст."
            ),
            "enrich": (
                "Стиль: РАСШИРЬ. Текст короткий — дополни его контекстом, поясни почему это важно для крипто-рынка. "
                "Сохрани все факты из оригинала, добавь честный аналитический комментарий."
            ),
        }.get(tone, "Стиль: нейтральный, информативный.")

        title_ctx = f'ЗАГОЛОВОК: "{title}"\n\n' if title else ""
        
        # Для enrich-тона: расширяем, а не сокращаем → задаём другой целевой диапазон длины
        if is_enrich:
            length_req = f"СТРОГО от {min(len(text) + 100, target_len - 100)} до {target_len} символов"
        elif is_digest:
            length_req = f"СТРОГО от 150 до {target_len} символов (это обязательно!)"
        else:
            length_req = f"СТРОГО от {target_len - 80} до {target_len} символов"

        # БАГ 14 ИСПРАВЛЕН: разные prompts для разных тонов
        # digest — компактный prompt без 3-частной структуры (они противоречили tone_instructions)
        if is_digest:
            prompt = f"""Ты — редактор и корректор крипто-канала BLEXLER. Сделай краткую выжимку новости.

{title_ctx}НОВОСТЬ:
{text[:2000]}

ЗАДАЧА: напиши ровно 2-3 коротких предложения на русском языке.

ВАЖНЫЕ ПРАВИЛА:
1. СТРОГО ЗАПРЕЩЕНО: дублировать заголовок или его смысл.
2. СТРОГО ЗАПРЕЩЕНО: если в новости всего 1-2 предложения, которые полностью покрыты заголовком — верни ПУСТУЮ СТРОКУ.
3. Структура: О чем новость + дополнительные детали или почему это важно.
{length_req}.
СТРОГО ЗАПРЕЩЕНО: любые списки, любые эмодзи, любые заголовки, жирный текст (**), курсив (__). Только чистый текст.

ВЫВЕДИ ТОЛЬКО ТЕКСТ (ИЛИ ПУСТУЮ СТРОКУ). Без пояснений."""
        else:
...
ТРЕБОВАНИЯ:
1. Длина результата: {length_req} (считай все символы включая пробелы)
2. {tone_instructions}
...
ВЫВЕДИ ТОЛЬКО ГОТОВЫЙ ТЕКСТ ПОСТА. Без пояснений, без кавычек вокруг текста."""
        ...
        try:
            ...
            # БАГ 12 ИСПРАВЛЕН: принудительно удаляем ** и __ которые ИИ мог оставить
            import re
            result_text = re.sub(r'\*{1,}', '', result_text)
            result_text = re.sub(r'_{1,}', '', result_text)
