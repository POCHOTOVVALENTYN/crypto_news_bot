...
{{
    ...
    async def rewrite_for_telegram(
        ...
        # БАГ 2 ИСПРАВЛЕН: разные prompts для разных тонов
        # digest — компактный prompt без 3-частной структуры (они противоречили tone_instructions)
        if is_digest:
            prompt = f"""Ты — редактор крипто-канала BLEXLER. Сделай краткую выжимку новости.
...
{length_req}.
БЕЗ списков, БЕЗ эмодзи, БЕЗ заголовков, только чистый текст.
